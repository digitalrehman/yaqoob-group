...
const AdminExpenseDetail = ({route, navigation}) => {
  const {from_date, to_date, account_type, title, total} = route.params || {};

  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [chartSeries, setChartSeries] = useState([{value: 1, color: COLORS.GREY}]);
  ...
  const fetchData = async (fromDt = startDate, toDt = endDate) => {
    try {
      ...
      console.log('Fetching Admin Data:', {from_date: formatDateForApi(fromDt), to_date: formatDateForApi(toDt), account_type});
  ...
  return (
    <View style={styles.container}>
      <SimpleHeader title={title || 'Administrative Expenses'} />
